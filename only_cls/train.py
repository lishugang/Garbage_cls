...
def train_one_epoch(epoch, net, train_loader, cirterion, optimizer, test_loader, best_eval_acc):
    train_correct = 0
    train_total = 0
    loss = 0
    for i, data in enumerate(train_loader):
        ...
        if args.test_batch:
            if i % args.test_batch_num == 0:
                net.eval()
                best_eval_acc = eval(net, test_loader, best_eval_acc)
...
def eval(net, test_loader, best_eval_acc):
    correct = 0
    test_total = 0
...
if __name__ == "__main__":
    ...
    train_dataset = MyDataSet(dataset_path=trainset_path, classes=classes, transform=train_transform)
    ...
    net.fc = nn.Linear(num_cls_ori, len(classes))
    net.to(device)
    ...
    for epoch in range(args.epochs):
        net.train()
        best_eval_acc = train_one_epoch(epoch, net, train_loader, cirterion, optimizer, test_loader, best_eval_acc)
        net.eval()
        best_eval_acc = eval(net, test_loader, best_eval_acc)
